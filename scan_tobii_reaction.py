...
dataFile = open('F:\Liudata\subj30\data_oddball\ ' + "%s.csv" % (info['num'] + '_' + info['name']), 'a')
...
class TobiiController:
    ...
    def findEyes(self):
        ...
        # Set default colors
        self.correctColor = (0, 1.0,0)
        self.mediumColor = (0, 1.0, 1.0)
        self.wrongColor = (1.0, 0, 0)

        # Make a dummy message
        self.findmsg = psychopy.visual.TextStim(self.win, color=1.0,
        ...
        self.eyeArea = psychopy.visual.Rect(self.win, lineColor=(1, 1, 1),
    ...
    def doCalibration(self, calibrationPoints=[(0.5, 0.5), (0.2, 0.8),
                                               (0.2, 0.2), (0.8, 0.8),
                                               (0.8, 0.2)],
        ...
        if calibrationPoints is None:
            calibrationPoints = [(0.5, 0.5), (0.2, 0.8),
        ...
        # Make a dummy message
        self.calmsg = psychopy.visual.TextStim(self.win, color= 1.0,
...
if __name__ == "__main__":
    import sys
    screen = psychopy.monitors.Monitor(name='tobiiTx300', width=51, distance=60)
    ...
    win = psychopy.visual.Window(fullscr=True, units='pix', monitor=screen, colorSpace='rgb')
    ...
    controller.setDataFile('F:\Liudata\subj30\data_oddball\data_block1.csv')
    ...
    # 指导语
    text_1 = psychopy.visual.TextStim(win, text=u'', height=48, font='Hei', pos=(0.0, 0.0), color='white')
    ...
    pic_list = []
    pic_ims = glob.glob("F:\Liuxu\psy\experiment\exp_reaction\image\pa\*.jpg")
    ...
    for jj in pic_list:
        # 十字注视点
        text_1 = psychopy.visual.TextStim(win, text=u'+', height=72, pos=(0.0, 0.0), color='white')
        text_1.draw()
        win.flip()
        core.wait(random.uniform(0.8, 1.2))         # 随机空屏

        trigger = os.path.split(jj)[1].split('.')[0]

        mark = int(trigger)

        Key_response = psychopy.event.clearEvents()  # 清除内存按键

        timer = core.Clock()

        # 呈现图片
        controller.recordEvent(trigger)  # 发送trigger
        send_code(mark)
        pic = psychopy.visual.ImageStim(win)
        pic.image = jj
        pic.draw()
        win.flip()
        ...
        Key_response = psychopy.event.getKeys(keyList=['f', 'j'], timeStamped=timer)

        try:
            Key_1 = Key_response[0][0]
            Key_2 = Key_response[0][1] + 2.0
        ...
        if count in list_rest:
            text_2 = psychopy.visual.TextStim(win, text=u'休息一下，按空格键继续', pos=(0.0, 0.0), wrapWidth=1.0, color='white', height=48, font='Hei')
            text_2.draw()
            win.flip()
            core.wait(0)
            k_2 = event.waitKeys()
    ...
    # 结束语
    text_3 = visual.TextStim(win, text=u'该部分实验结束，请休息一下。', pos=(0.0, 0.0), wrapWidth=1.0, color='white', height=48, font='Hei')
    text_3.draw()
    win.flip()
    core.wait(3)
